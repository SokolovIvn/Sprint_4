...
class MainPage:
    ...
    def scroll_to_list(self):
        element = self.driver.find_element(*main_page.head_list_home_subHeader)
        self.driver.execute_script("arguments[0].scrollIntoView();", element)

    @allure.step("прокручиваем страницу до кнопки Заказа в теле страницы")
    def scroll_to_button_ortder(self):
        element = self.driver.find_element(*main_page.order_button)
        self.driver.execute_script("arguments[0].scrollIntoView();", element)
    ...
    def press_button_order(self):
        self.driver.find_element(*main_page.order_button).click()

    @allure.step("ожидаем картинку самоката на главной")
    def wait_print_on_main(self):
        WebDriverWait(self.driver, 5).until(expected_conditions.visibility_of_element_located(main_page.image_on_main_page))
